...
if ($_SERVER['REQUEST_METHOD'] == 'POST') { // Handle the form.
	...
	if (preg_match ('/^\w{4,20}$/', $trimmed['password1']) ) {
		...
		echo '<p class="alert alert-danger">Please enter a valid password! Use only letters, numbers, and the underscore. Must be between 4 and 20 characters long.</p>';
	}
	...
	if ($fn && $ln && $e && $p) { // If everything's OK...
		...
		if (mysqli_num_rows($r) == 0) { // Available.
			...
			if (mysqli_affected_rows($dbc) == 1) { // If it ran OK.
				...
				echo '<p class="alert alert-success">here is your activation link fo tyour account';
				echo '<a href="' . $body . '" title="link">Activate</a>';
				echo '<p>';
				
				// Finish the page:
				echo '<h3 class="alert alert-success">Thank you for registering! Please click on the link in order to activate your account.</h3>';
			...
			echo '<p class="alert alert-danger">That email address has already been registered. If you have forgotten your password, use this link ';
			echo '<a class="btn btn-primary" href= "forgot_password.php" >forgot password</a></p>';
		}
